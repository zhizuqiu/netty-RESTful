...
public class MethodData {
     ...
     * 存储restful method
     */
    private static final Map<RestMethodKey, RestMethodValue> REST_METHOD_MAP = new HashMap<>();
    /**
     * 存储template method
     */
    private static final Map<String, TemplateMethodValue> TEMPLATE_METHOD_MAP = new HashMap<>();
    /**
     * 存储静态资源
     */
    private static final Map<RestMethodKey, ResourceValue> RESOURCES_MAP = new HashMap<>();
    /**
     * 存储拦截器
     */
    ...
    public static ResourceValue putResource(RestMethodKey key, ResourceValue resource) {
        return RESOURCES_MAP.put(key, resource);
    }

    public static Set<RestMethodKey> resourcesKeySet() {
        return RESOURCES_MAP.keySet();
    }
    ...
    /**
     * 从RESOURCES_MAP获取html，并且进行模板填充
     */
    public static String getResourceAndTemplate(RestMethodKey key) {
        ResourceValue resourceValue = RESOURCES_MAP.get(key);
        if (resourceValue == null) {
            return null;
        }
        String value = resourceValue.getValue();
        value = templateFromUrl(key.getUrl(), value);
        return value;
    }

    /**
     * 根据path从TEMPLATE_METHOD_MAP中获取模板参数，并填充到value中
     */
    public static String templateFromUrl(String path, String value) {
        ...
        if (templateMethodValue != null) {
            ...
            if (re != null) {
                value = templateMustache(re, value);
            }
        }
        return value;
    }
